...
switch($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			pe_token_match();
			if (!$_p_user_tname) pe_error('请填写收货人');
			if (!pe_formcheck('phone', $_p_user_phone)) pe_error('请填写正确的手机号');
			if (!$_p_address_province) pe_error('请选择省份');
			if (!$_p_address_city) pe_error('请选择城市');
			if (!$_p_address_area) pe_error('请选择区县');
			if (!$_p_address_text) pe_error('请填写详细地址');
			...
			if ($db->pe_insert('useraddr', pe_dbhold($sql_set))) {
				pe_success('添加成功！', 'user.php?mod=useraddr');
			}
			else {
				pe_error('添加失败');
			}
	...
	case 'edit':
		...
		if (!$info['address_id']) pe_error('非法操作');
		if (isset($_p_pesubmit)) {
			pe_token_match();
			if (!$_p_user_tname) pe_error('请填写收货人');
			if (!pe_formcheck('phone', $_p_user_phone)) pe_error('请填写正确的手机号');
			...
			if ($db->pe_update('useraddr', array('address_id'=>$address_id), pe_dbhold($sql_set))) {
				pe_success('修改成功！', 'user.php?mod=useraddr');
			}
			else {
				pe_error('修改失败');
			}
