...
switch ($act) {
	...
	case 'add':
		...
		if (isset($_p_pesubmit)) {
			...
			if (!is_array($userbank_list[$_p_userbank_id])) pe_error('请选择收款账户');
			if ($cashout_money < $cache_setting['cashout_min']) pe_error("提现金额小于{$cache_setting['cashout_min']}元");
			if ($cashout_money > $info['user_money']) pe_error('余额不足');
			...
			if ($db->pe_insert('cashout', pe_dbhold($sql_set))) {
				...
				pe_success('提交成功，等待审核！', 'user.php?mod=cashout');
			}
			else {
				pe_error('提交失败');
			}
	...
	case 'del':
		...
		if (!$info['cashout_id']) pe_error('参数无效');
		...
		if ($db->pe_update('cashout', array('cashout_id'=>$info['cashout_id']), $sql_set)) {
			...
			add_moneylog($info['user_id'], 'add', $cashout_money, "用户取消提现，退回{$cashout_money}元");
			pe_success('取消成功！');
		}
		else {
			pe_error('取消失败');
