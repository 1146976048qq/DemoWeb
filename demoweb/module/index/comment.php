...
switch ($act) {
	...
	default:
		...
		if (isset($_g_page)) {
			...
			$info_list = $db->pe_selectall('comment', $sql_where, 'user_name,user_logo,comment_id,comment_star,comment_logo,comment_text,comment_atime', array('20', $_g_page));
			foreach ($info_list as $k=>$v) {
				$info_list[$k]['comment_star'] = pe_comment($v['comment_star'], 14);
				$info_list[$k]['comment_atime'] = pe_date($v['comment_atime'], 'Y-m-d');
				$info_list[$k]['user_logo'] = pe_thumb($v['user_logo'], _90, _90, 'avatar');
				//评价晒图
				if ($v['comment_logo']) {
